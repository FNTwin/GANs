...
class GAN():
	def __init__(self, epochs, latent_dimension, lr, bs, gpu=True):
		self.g=Generator(latent_dimension)
	...
	def train(self,dataloader):
		...
		for epoch in range(self.epochs):
		    for count, (j,labels) in enumerate(dataloader):
		        ...
		        valid = Variable(Tensor(j.size(0), 1).fill_(1.0))
        		fake = Variable(Tensor(j.size(0), 1).fill_(0.0))
		       
		        #Generator training (zero gradient, predict, backpropagation, adjust weight
		        self.optimizer_g.zero_grad()
		        fake_images=self.g(z.to(self.device))
		        valid=self.d(fake_images.to(self.device))
		        ...
		        real= self.loss(self.d(j.to(self.device)), Variable(torch.ones(j.size(0))).unsqueeze(1).to(self.device))
		        fake= self.loss(self.d(fake_images.detach().to(self.device)),Variable(torch.zeros(j.size(0))).unsqueeze(1).to(self.device))
		        d_loss=(real+fake)/2
		        d_loss.backward()
